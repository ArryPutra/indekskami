...
<aside x-cloak x-data="{ isOpen: false }" class="bg-white top-24 fixed w-full px-6 max-md:px-0 z-20">
    <svg x-show="!isOpen" @click="isOpen = true" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
        viewBox="0 0 24 24" class="fixed top-6 size-12 md:hidden left-2">
    ...
    <div :class="{ 'max-md:-flex': isOpen, 'max-md:hidden': !isOpen }"
        ...
        <a href="{{ route('responden.evaluasi.i-kategori-se') }}"
            class="{{ request()->routeIs('responden.evaluasi*') ? $activeMenuClass : false }} py-4 md:px-2 max-md:pl-4">Evaluasi</a>
