...
@section('content')
    ...
    <form method="POST" class="flex flex-col gap-2" action="{{ $page_meta['route'] }}">
        ...
        <x-text-field name="nama" label="Nama Instansi" placeholder="Masukkan nama instansi"
            value="{{ old('nama', $responden?->user?->nama) }}" />
        <x-dropdown name="daerah" label="Pilih Daerah Instansi">
            <x-dropdown.option value="Kabupaten/Kota" :selected="old('daerah', $responden->daerah) == 'Kabupaten/Kota'">Kabupaten/Kota</x-dropdown.option>
            <x-dropdown.option value="Provinsi" :selected="old('daerah', $responden->daerah) == 'Provinsi'">Provinsi</x-dropdown.option>
        </x-dropdown>
        <x-text-field name="username" label="Username" placeholder="Masukkan username"
            value="{{ old('username', $responden?->user?->username) }}" />
        <x-text-field name="email" label="Email" placeholder="Masukkan email"
            value="{{ old('email', $responden?->user?->email) }}" />
        <x-text-field name="nomor_telepon" label="Nomor Telepon" placeholder="Masukkan nomor telepon"
            value="{{ old('nomor_telepon', $responden?->user?->nomor_telepon) }}" />
        <x-text-area name="alamat" label="Alamat" placeholder="Masukkan alamat"
            value="{{ old('alamat', $responden->alamat) }}" />
        ...
        <x-radio label="Apakah responden dapat melakukan evaluasi?">
            <x-radio.option name="akses_evaluasi" id="ya" value="1" :checked="old('akses_evaluasi', $responden->akses_evaluasi) == '1'">
                Ya
            </x-radio.option>
            <x-radio.option name="akses_evaluasi" id="tidak" value="0" :checked="old('akses_evaluasi', $responden->akses_evaluasi) == '0'">
                Tidak
            </x-radio.option>
        </x-radio>
