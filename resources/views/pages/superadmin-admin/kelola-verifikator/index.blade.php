            ...
            @if (count($daftarVerifikator) > 0)
                @foreach ($daftarVerifikator as $index => $verifikator)
                    <x-table.tr>
                        ...
                        <x-table.td class="font-semibold">{{ $verifikator->user->nama }}</x-table.td>
                        <x-table.td>{{ $verifikator->user->username }}</x-table.td>
                        <x-table.td>{{ $verifikator->user->email }}</x-table.td>
                        <x-table.td>{{ $verifikator->user->nomor_telepon }}</x-table.td>
                        <x-table.td>{{ $verifikator->nomor_sk }}</x-table.td>
                        <x-table.td>
                            <div class="flex gap-2 flex-wrap">
                                ...
                                @if ($verifikator->user->apakah_akun_nonaktif == false)
                                    <form id="formNonaktifkanVerifikator-{{ $verifikator->user->id }}"
                                        action="{{ route('kelola-verifikator.destroy', $verifikator->user->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <input type="hidden" name="apakah_akun_nonaktif" value="true">
                                        <x-button type="submit" color="red"
                                            onclick="nonaktifVerifikator('{{ $verifikator->user->nama }}', {{ $verifikator->user->id }})">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-5">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                @else
                                    <form id="formAktifkanVerifikator-{{ $verifikator->user->id }}"
                                        action="{{ route('kelola-verifikator.destroy', $verifikator->user->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        ...
                                        <x-button type="submit" color="green"
                                            onclick="aktifVerifikator('{{ $verifikator->user->nama }}', {{ $verifikator->user->id }})">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-5">
                                                <path stroke-linecap="round" stroke-linejoin="round"
