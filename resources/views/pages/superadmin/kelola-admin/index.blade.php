...
@section('content')
    <div class="grid lg:grid-cols-3 md:grid-cols-2 gap-4 mb-8">
        ...
        <x-card>
            ...
            <x-slot:value>{{ $daftarDataCard['totalAdmin'] }}</x-slot:value>
        </x-card>
    </div>

    <form method="GET" action="{{ route('kelola-admin.index') }}">
        <section class="flex justify-between flex-wrap gap-4 mb-4">
            <div class="flex gap-3 w-full">
                <x-text-field value="{{ request('cari') }}" type="text" name="cari" placeholder="Cari admin" />
    ...
    <x-table>
        <x-table.thead>
            ...
            <x-table.th>Nama Admin</x-table.th>
        ...
        <x-table.tbody>
            @if (count($daftarAdmin) > 0)
                @foreach ($daftarAdmin as $index => $admin)
                    <x-table.tr>
                        <x-table.td>{{ ($daftarAdmin->currentPage() - 1) * $daftarAdmin->perPage() + $index + 1 }}</x-table.td>
                        <x-table.td class="font-semibold">{{ $admin->user->nama }}</x-table.td>
                        <x-table.td>{{ $admin->user->username }}</x-table.td>
                        <x-table.td>{{ $admin->user->email }}</x-table.td>
                        <x-table.td>{{ $admin->user->nomor_telepon }}</x-table.td>
                        <x-table.td>
                            <div class="flex gap-2 flex-wrap">
                                <x-button href="{{ route('kelola-admin.show', $admin->id) }}">
                                ...
                                <x-button href="{{ route('kelola-admin.edit', $admin->id) }}" color="blue">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="size-5">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                @if ($admin->user->apakah_akun_nonaktif == false)
                                    <form id="formNonaktifkanAdmin-{{ $admin->user->id }}"
                                        action="{{ route('kelola-admin.destroy', $admin->user->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <input type="hidden" name="apakah_akun_nonaktif" value="true">
                                        <x-button type="submit" color="red"
                                            onclick="nonaktifAdmin('{{ $admin->user->nama }}', {{ $admin->user->id }})">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-5">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                @else
                                    <form id="formAktifkanAdmin-{{ $admin->user->id }}"
                                        action="{{ route('kelola-admin.destroy', $admin->user->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <input type="hidden" name="apakah_akun_nonaktif" value="false">
                                        <x-button type="submit" color="green"
                                            onclick="aktifAdmin('{{ $admin->user->nama }}', {{ $admin->user->id }})">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="size-5">
                                                <path stroke-linecap="round" stroke-linejoin="round"
...
@push('script')
    ...
    <script>
        function nonaktifAdmin(nama, userId) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const form = document.getElementById('formNonaktifkanAdmin-' + userId);
                    form.submit();
                }
            });
        }

        function aktifAdmin(nama, userId) {
            window.event.preventDefault();
            Swal.fire({
                title: "Apakah Anda yakin?",
                ...
                if (result.isConfirmed) {
                    const form = document.getElementById('formAktifkanAdmin-' + userId);
                    form.submit();
                }
            });
        }
